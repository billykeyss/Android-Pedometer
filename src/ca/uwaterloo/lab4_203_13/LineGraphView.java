...
public class LineGraphView extends View {
	...
	public final int[] defalutColors = { 0xffff0000, 0xff00ff00, 0xff0000ff,
	 ...
	 * 
	 * @param context
	 *            The application context. You can get your application context
	 *            by calling getApplicationContext() from your Activity
	 * @param dataWidth
	 *            How many data points to display before the graph starts
	 *            scrolling
	 * @param labels
	...
	private void prepForData(Canvas canvas) {
		canvas.drawLine(0, HEIGHT / 2, WIDTH + AXIS_WIDTH, HEIGHT / 2,
				graphPaint);
		...
		canvas.drawText("-" + Float.toString(maxY) + " m/s^2", 0, HEIGHT,
				graphPaint);

		for (int i = 0; i < labels.size(); i++) {
			canvas.drawText(labels.get(i) + ":", 0, 30 + i * 20, graphPaint);
			canvas.drawLine(0, 35 + i * 20, AXIS_WIDTH - 20, 35 + i * 20,
					linePaints.get(i));
		}
	}

	private void drawLine(Canvas canvas, int count, Float rawSrc,
			Float rawDest, Paint paint) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 1; i < points.size(); i++) {
			for (int j = 0; j < points.get(i).length; j++) {
				drawLine(canvas, i, points.get(i - 1)[j], points.get(i)[j],
						linePaints.get(j));
			}
		}
	...
	/**
	 * Adds a set of datapoints for the next x value. The data points should be
	 * in the same order as the array of labels this object was initialized
	 * with.
	 * 
	 * @param y
